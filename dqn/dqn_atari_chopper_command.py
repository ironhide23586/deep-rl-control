...
class DQNEnvironment:
    ...
    def __init__(self, env_name="ChopperCommand-v0", num_lives=3, flicker_buffer_size=2,
                 ...
                 rewards_data_cache_fpath='rewards_history.pkl'):
    ...
    def plot_rewards(self):
        ...
        plt.legend(loc='best')
        # plt.xticks(np.linspace(0, self.plot_frame_indices[-1], len(self.plot_frame_indices)).astype(np.int))
        # plt.yticks(np.linspace(0, max([max(self.curr_episode_rewards), max(self.best_episode_rewards),
        #                                max(self.ema_episode_rewards)]), 10))
