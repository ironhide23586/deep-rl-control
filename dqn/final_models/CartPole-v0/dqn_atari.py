...
class DQNEnvironment:
    ...
    def update_random_action_prob(self):
        ...
        if self.num_episodes % 100 == 0:
            eps = self.end_epsilon + (self.start_epsilon - self.end_epsilon) * \
                  np.exp(-1. * self.curr_train_step / 5000)
            print(eps)
        else:
            eps = self.random_action_prob
        self.random_action_prob = eps
